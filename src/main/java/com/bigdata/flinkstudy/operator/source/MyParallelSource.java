...
/**
 *  用户自定义并行数据源（实现SourceFunction<T>接口，T是输出类型）
 *  如果需要支持并行度的数据源就要实现（ParallelSourceFunction接口，其余代码不变）
 */
public class MyParallelSource implements SourceFunction<Event> {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 添加自定义数据源
        DataStreamSource<Event> myStream = env.addSource(new MyParallelSource());
        myStream.print();
